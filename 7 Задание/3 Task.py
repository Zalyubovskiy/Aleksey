"""
Задание-3. Реализовать программу работы с органическими
клетками. Необходимо создать класс Клетка. В его конструкторе
...
уменьшение, умножение и обычное (не целочисленное) деление
клеток, соответственно. В методе деления должно осуществляться
...
только если разность количества ячеек двух клеток больше
...
class Cell:
    ...
    def __add__(self, other):
        return f"Сложение: {self.quant + other.quant}"

    def __sub__(self, other):
        sub = self.quant - other.quant
        return f"Вычитание: {sub}" if sub > 0 else "Клетка отсутствует"

    def __mul__(self, other):
        return f"Умножение: {self.quant * other.quant}"

    def __truediv__(self, other):
        return f"Деление: {self.quant // other.quant}"

    def make_order(self, num):
        total = ""
        for x in range(int(self.quant / num)):
            total += "*" * num + "\n"
        total += "*" * (self.quant % num) + "\n"
        return total
